<template>
  <div v-if="field" class="py-2">
    ...
    <div v-if="field.type === 'number'" class="border-b py-2 px-4">
      ...
      <v-checkbox v-model="field.is_rating" class="mt-3" :name="field.id + '_is_rating'" @update:model-value="initRating">
        Rating
      </v-checkbox>
      <p class="text-gray-400 mb-3 text-xs">
      ...
      <text-input v-if="field.is_rating" name="rating_max_value" native-type="number" :min="1" class="mt-3" :form="field"
        required label="Max rating value" />

      <v-checkbox v-model="field.is_scale" class="mt-4" :name="field.id + '_is_scale'" @update:model-value="initScale">
        Scale
      </v-checkbox>
      ...
      <v-checkbox v-model="field.is_slider" class="mt-4" :name="field.id + '_is_slider'" @update:model-value="initSlider">
        Slider
      </v-checkbox>
      <p class="text-gray-400 text-xs mb-5">
    ...
    <div v-if="['select', 'multi_select'].includes(field.type)" class="border-b py-2 px-4">
      ...
      <!-- Dropdown for selecting option type -->
      <select v-model="field.selectedOptionType" @change="onOptionTypeChange" class="border px-10">
      ...
      <div v-if="field.selectedOptionType === 'custom'">
        <text-area-input v-model="optionsText" :name="field.id + '_options_text'" class="mt-3"
          label="Set selection options" help="Add one option per line" @update:model-value="onFieldOptionsChange" />
      </div>

      <div v-else>
        <!-- Set the relevant key and name based on selected option type -->
        <input type="hidden" :value="field.selectedOptionType" />
        <input type="hidden" :value="getFieldName(field.selectedOptionType)" />

        <!-- Fields for 'store' option -->
        <div v-if="field.selectedOptionType === 'store'">
          <input v-model="field.storeField" type="text" @change="onFieldChange" hidden/>
        </div>

        <!-- Fields for 'user' option -->
        <div v-else-if="field.selectedOptionType === 'user'">
          <input v-model="field.userField" type="text" @change="onFieldChange" hidden/>
        </div>

        <!-- Fields for 'product' option -->
        <div v-else-if="field.selectedOptionType === 'product'">
          <input v-model="field.productField" type="text" @change="onFieldChange" hidden/>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    onOptionTypeChange() {
      // Reset custom fields when the option type changes
      if (this.field.selectedOptionType !== 'custom') {
        this.field.Field_Name = this.getFieldName(this.field.selectedOptionType);
        this.field.Field_Id = this.getFieldId(this.field.selectedOptionType);
      }
    },
    onFieldChange() {
      this.field.multi_select.options = [{
        selectedName: this.field.selectedName,
        storeField: this.field.storeField,
      }];
    },
    getFieldName(optionType) {
      switch (optionType) {
        case 'store':
          return 'store_name';
        case 'user':
