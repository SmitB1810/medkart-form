        ...
        foreach ($data as $answerKey => $answerValue) {
            ...
            if (
                ...
                if ($field['type'] == 'text' && isset($field['generates_uuid']) && $field['generates_uuid']) {
                    $finalData[$field['id']] = (true) ? Str::uuid() : "Please upgrade your OpenForm subscription to use our ID generation features";
                } else {
                    if ($field['type'] == 'text' && isset($field['generates_auto_increment_id']) && $field['generates_auto_increment_id']) {
                        $finalData[$field['id']] = (true) ? (string) ($this->form->submissions_count + 1) : "Please upgrade your OpenForm subscription to use our ID generation features";
                    } else {
